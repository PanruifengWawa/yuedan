...
// GetTokenByTokenStr retrieves Token by TokenStr. Returns error if
// any error occurs
func GetTokenByTokenStr(tokenStr string) (v *Token, err error) {
	o := orm.NewOrm()
	v = &Token{}
	if err = o.QueryTable("token").Filter("token_str", tokenStr).One(v); err == nil {
		return v, nil
	}
	return nil, err
}

// GetTokenByUserId retrieves Token by UserId. Returns error if
// any error occurs
func GetTokenByUserId(userId int) (v *Token, err error) {
	o := orm.NewOrm()
	v = &Token{}
	if err = o.QueryTable("token").Filter("user_id", userId).One(v); err == nil {
		return v, nil
	}
	return nil, err
}
