...
type Order struct {
	...
	Place       string    `orm:"column(place);size(128)"`
	Pay         string    `orm:"column(pay);size(64)"`
	Content     string    `orm:"column(content)"`
	UserId      *User     `orm:"column(user_id);rel(fk)"`
	Number      int       `orm:"column(number)"`
	Sex         string    `orm:"column(sex);size(1)"`
}
...
func GetOrderByUserId(userId int, offset int64, limit int64) (orders []interface{}, err error) {
	o := orm.NewOrm()
	var maps []orm.Params
	_, err = o.QueryTable("order").Filter("user_id", userId).OrderBy("-ReleaseDate").Limit(limit, offset).Values(&maps, "Id", "Title", "Type", "Date", "ReleaseDate", "Place", "Pay", "Content", "Number", "Sex", "UserId__Id", "UserId__Nickname")
	if err == nil {
		for _, m := range maps {
			m["UserId"] = m["UserId__Id"]
			m["NickName"] = m["UserId__Nickname"]
			delete(m, "UserId__Id")
			delete(m, "UserId__Nickname")
...
func GetOrderCountByUserId(userId int) (count int64, err error) {
	o := orm.NewOrm()

	count, err = o.QueryTable("order").Filter("user_id", userId).Count()
	return count, err
